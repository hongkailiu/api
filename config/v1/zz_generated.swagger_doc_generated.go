...
var map_ClusterVersionStatus = map[string]string{
	...
	"conditionalUpdateRisks": "conditionalUpdateRisks represents issues for ConditionalUpdates. The cluster-version operator will evaluate all risks associated to a conditional update when it is the desired update and only accept it if all its associated risks are in \"spec.desiredUpdate.accept\".",
}
...
var map_ConditionalUpdate = map[string]string{
	...
	"riskNames":  "riskNames represents names of status.conditionalUpdateRisks that are associated with the release in a ConditionalUpdate. The cluster-version operator will evaluate these risks and only accept the update if there is at least one risk and for every risk it is either not applied to the cluster or considered acceptable by the cluster administrator.",
	"risks":      "risks represents the range of issues associated with updating to the target release. The cluster-version operator will evaluate all entries, and only recommend the update if there is at least one entry and all entries recommend the update. DEPRECATED: the risks has been deprecated by riskNames and will be removed in a future release.",
...
var map_ConditionalUpdateRisk = map[string]string{
	...
	"conditions":    "conditions represents the observations of the conditional update risk's current status. Known types are: * Apply, for whether the risk is applied to the current cluster.",
...
var map_Update = map[string]string{
	...
	"accept":       "accept allows an administrator to specify the names of ConditionalUpdateRisk those are considered acceptable. A conditional update is accepted by Cluster-Version operator only if all of its risks are acceptable.",
}
...
var map_UpdateHistory = map[string]string{
	...
	"acceptedRisks":  "acceptedRisks records risks which were accepted to initiate the update. For example, it may mention an Upgradeable=False or missing signature that was overridden via desiredUpdate.force, or an update that was initiated despite not being in the availableUpdates set of recommended update targets, or in the conditionUpdates set and all associated risks are specified in \"spec.desiredUpdate.accept\".",
}
